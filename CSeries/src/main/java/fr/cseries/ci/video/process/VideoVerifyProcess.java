		...
		while (true) {
			if (!queue.getVideosToProcess().isEmpty()) {
				...
				if (video.getVideoFile().exists()) {
					...
					if(video.getVideoFile().delete()) System.out.println("[" + queue.getId() + "] L'ancien fichier " + video.getVideoFile().getAbsolutePath() + " a été supprimé !");
					else System.out.println("[" + queue.getId() + "] L'ancien fichier " + video.getVideoFile().getAbsolutePath() + " n'a pas pû être supprimé !");
				}
